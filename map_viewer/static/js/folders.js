...
function mapViewerChange() {
    listFiles = $("#csvFile").prop("files")
    if (listFiles.length == 0) {
        $("#csvFile-text").val("No file selected")
    }
    csv = localStorage.getItem("csv");
    if (csv != null) {
        $("#csvFile-text").val(localStorage.getItem("csv"));
    }
    $("#csvFile").on("change", function () {
        listFiles = $("#csvFile").prop("files")
        if (listFiles[0].type == "text/csv") {
            file = listFiles[0]
            localStorage.setItem("csv", file.name);
            $("#csvFile-text").val(file.name)
            readFile(file);
            setTimeout(function () {
                // map.off();
                // map.remove();
                // loadMap()
                newdata = JSON.parse(localStorage.getItem("map"));
                heatmapLayer.setData(newdata);
            }, 1000);

        } else {
            alert("the file extension does not match the csv type")
        }
    });
}
...
function readFile(file) {
    ...
    reader.onload = function () {
        ...
        jsonData = csvToJSON(mapViewerCSV);
        newData = dataTransformer(jsonData)
        localStorage.setItem("map", JSON.stringify(newData));
    };

    reader.onerror = function () {
        localStorage.setItem("map", reader.error)
    };
}
...
function dataTransformer(json) {
    var data = json.data;
    var headers = json.headers;
    var transformedData = [];
    for (var i = 0; i < data.length; i++) {
        var row = data[i];
        newLat = parseFloat(row[2]);
        newLng = parseFloat(row[3]);
        var newRow = { lat: newLat, lng: newLng, count: 3 };
